...
date: "Fecha de actualización: 2 de febrero de 2021"
...
f1<-ggplot(datos1, aes(trim, const, fill = tipo)) +
    geom_col(width=0.1)+
  scale_fill_manual(values=c("#E69F00","#999999"))+
  theme_minimal()+
  theme(legend.title = element_blank(),
        axis.text.x = element_text(angle = 90, vjust = 0.5))+
  scale_x_continuous("Trimestre",breaks=seq(2000,2021,by=2))
...
#plot_ly(datos, x = ~trim, y = ~const, type = 'bar', 
 #       split = ~tipo, color=~tipo,colors = c("#feb24c", "#bdbdbd"),
  ...
  layout(title=list(text="<b>Producto Interno Bruto de la Construcción</b><br><i>2000-2021</i><br><i>Variación % anual</i>", 
         x = 0,
...
df<-fortify(fcast)%>%
  ...
  mutate(Index=as.yearqtr(Index,format="%Y-%m-%d"))

##Gráfica de ggplot
f2<-ggplot(df, aes(x=Index, Data)) + 
...
ggplotly(f2,width = 900, height = 500)%>%
  layout(title=list(text="<b>Trabajadores de la construcción asegurados ante el IMSS</b><br><i>1999-2021</i><br><i>Variación % anual</i>", 
         x = 0,
         font=list(family = "Century Gothic",
        size = 16,
        color = 'black')),
        ...
        dragmode="pan",
         annotations =
 list(x = 0, y = 0, text = "Fuente: Elaborado por CANADEVI Nacional. Gerencia de Fondos de Vivienda. Coordinación de Indicadores de Vivienda con información del IMSS e INEGI.", 
      showarrow = F, xref='paper', yref='paper', 
      xanchor='left', yanchor='auto', xshift=0, yshift=0,
     font=list(size=8, color="black")))
